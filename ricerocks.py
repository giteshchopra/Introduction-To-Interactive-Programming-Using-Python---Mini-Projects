...
soundtrack = simplegui.load_sound("https://www.dropbox.com/s/czcw52tmme2aarz/Arc%20-%20Mind%20Vortex-Asphalt%208-Airborne%20OST-%20-%20Copy.ogg?dl=1")
...
class Sprite:
    ...
    def draw(self, canvas):
        ...
        if self.animated:
            image_center = [0, 0]
            image_center[0] = self.image_center[1] + self.age *self.image_size[0]
            image_center[1] = self.image_center[1]
    ...
    def collision(self,other_object):
        selfp=self.get_pos()
        objp=other_object.get_pos()
        selfr=self.get_rad()
        objr=other_object.get_rad()
        d=math.sqrt(math.pow((selfp[0]-objp[0]),2)+math.pow((selfp[1]-objp[1]),2))
        R=selfr+objr
        if(d<R):
            return True
        else:
...
def group_collide(group1,obj):
    global explosion_group
    image=random.choice([explosion_image1,explosion_image2,explosion_image3,explosion_image4])
    for a in list(group1):
        if(a.collision(obj)):
            group1.remove(a)
            explosion_group.add(Sprite(a.get_pos(),[0,0],0 ,0, image, explosion_info))
...
def sprite_process_group(a,canvas):
    for x in list(a):
        x.draw(canvas)
        if x.update():
            a.remove(x)
...
def draw(canvas):
    global time, started,rock_group,my_ship,score,miss_group,lives
    ...
    sprite_process_group(rock_group,canvas)
    sprite_process_group(miss_group,canvas)
    sprite_process_group(explosion_group,canvas)
    score=score+group_group_collide(rock_group,miss_group)
...
def rock_spawner():
    ...
    if(len(rock_group)<12):
        ...
        img=random.choice([asteroid_image1,asteroid_image2,asteroid_image3])
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, img, asteroid_info)
...
a_rock = Sprite([WIDTH / 3, HEIGHT / 3], [1, 1], 0, .1, img, asteroid_info)
...
timer.start()
frame.start()
print "Don't know why but the soundtrack takes time to load please be patient or load again"
